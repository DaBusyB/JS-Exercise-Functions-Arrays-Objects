...
function temperatureInF(temperature, unit) {
 ...
 return(unit === "F" ? temperature + unit : (Math.round(temperature * (9/5) + 32) + "F"))
}
...
function isItAnApple(arr) {
  for(let i = 0; i < arr.length; i++) {
    return(arr[i].includes('apple') ? true : false)
  }
}
...
*/
function getCarInfoByIndex(data, index) {
  let car = data[index]
...
function carMaker(odometer) {
  const odometerObj = {
    odometerNum: odometer,
    drive: function(distance) {
      this.odometerNum += distance
      return this.odometerNum 
    }
  }
